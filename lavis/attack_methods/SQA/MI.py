...
def MI_Attack(model, modalities, samples, norm=np.inf, iterations=5):
    eps = 8
    alpha = 2
    temperature = 0.2
    weight = {}
    ...
    for t in range(iterations):
        ...
        for mod in modalities:
            eta = torch.zeros_like(samples[mod]).uniform_(-1.0, 1.0) if t == 0 else torch.zeros_like(samples[mod])
            if mod == 'rgb':
        ...
        for mod in modalities:
            ...
            if mod == 'rgb':
                perturb = alpha_tensor.view(1, 1, -1, 1, 1) * torch.sign(g[mod])
            else:
                perturb = alpha_tensor.view(1, -1, 1, 1, 1) * torch.sign(g[mod])
