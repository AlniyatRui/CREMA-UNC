...
@registry.register_model("crema_flow")
class CREMA_FLOW(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        missing_mode: 
        0 for all available
        1 for vt + x (self.non_rgb_modalities[0])
        2 for vt + xy
        3 for vt + xyz 
        '''
        self.non_rgb_modalities = [modal for modal in modalities if modal != 'rgb']

        self._init_flow_models(encoder_config.hidden_size)
    ...
    def init_modality_components(self, encoder_config, num_features):
        if 'rgb' in self.modalities:
            self.query_tokens_rgb = nn.Parameter(
                torch.zeros(1, num_query_token, encoder_config.hidden_size))
            self.query_tokens_rgb.data.normal_(mean=0.0, std=encoder_config.initializer_range)
    ...
    def _init_flow_models(self, hidden_size):
        ...
        self.flow_v = Glow(in_channel=hidden_size, 
                        n_flow=32, 
                        n_block=1, 
                        affine=True, 
                        conv_lu=False
                    )
        self.flow_t = Glow(in_channel=self.t5_model.model_dim, 
                        n_flow=32, 
                        n_block=1, 
                        affine=True, 
                        conv_lu=False
        ...
        if self.missing_mode == 1:
            cat_input_size = 2 
            self.flow_x = Glow(in_channel=hidden_size, 
                        n_flow=32, 
                        n_block=1, 
                        affine=True, 
                        conv_lu=False
                    )
            self.cat_x = nn.Conv1d(hidden_size * cat_input_size, hidden_size, kernel_size=1, padding=0)
            self.rec_x = nn.Sequential(
                nn.Conv1d(hidden_size, hidden_size * cat_input_size, 1),
                Group(num_channels=hidden_size * cat_input_size, num_blocks=20, reduction=16),
                nn.Conv1d(hidden_size * cat_input_size, hidden_size, 1)
            )
            self.prior = ZeroConv2d(hidden_size, hidden_size * cat_input_size)
        else:
    ...
    def forward(self, samples):
        ...
        comb_random = 0.6
        if self.missing_mode == 1 and comb_random < 0.5:
            # case1: vtx, 50% x is missing, encode v here
            modal = 'rgb'
            input = samples[modal]
            if input.shape[1] == 3:
                input = input.permute(0, 2, 1, 3, 4)
            input_embed_dict[modal], input_att_dict[modal] = self.encode_input(input, modal)
        elif self.missing_mode == 0 or (self.missing_mode == 1 and comb_random >= 0.5):
        ...
        torch.save(input_embed_dict,'input_embed_dict.pt')
        exit(-1)
        ...
        # input_text_embeds [B, L, D] -> [B, D, L, 1]
        # [to do]: check unified prior
        # _, logdet_t, z_outs_t = self.flow_t(input_text_embeds.permute(0, 2, 1).unsqueeze(-1)) 
        log_p_t, logdet_t, z_outs_t = self.flow_t(input_text_embeds.permute(0, 2, 1).unsqueeze(-1)) 
        z_t = z_outs_t
        z_outs_t = z_outs_t[0]
        #  normalizing flow for vision
        # [to do]: check unified prior
        # _, logdet_v, z_outs_v = self.flow_v(input_embed_dict['rgb'].unsqueeze(-1))
        log_p_v, logdet_v, z_outs_v = self.flow_v(input_embed_dict['rgb'].unsqueeze(-1))
        z_v = z_outs_v
        z_outs_v = z_outs_v[0]
        
        # [to do]: check unified prior
        # zero = torch.zeros_like(z_outs_t).unsqueeze(0)
        # mean, log_sd = self.prior(zero).chunk(2, 1) 
        # log_p_sum_t = gaussian_log_p(z_outs_t.unsqueeze(0), mean, log_sd).view(input_text_embeds.size(0), -1).sum(1)
        # log_p_sum_v = gaussian_log_p(z_outs_v.unsqueeze(0), mean, log_sd).view(input_text_embeds.size(0), -1).sum(1)
        # log_p_t = logdet_t.sum() + log_p_sum_t
        log_p_t = torch.max(torch.zeros_like(log_p_t),
                            (-log_p_t / (np.log(2) * input_text_embeds.size(0) * input_text_embeds.size(1) * input_text_embeds.size(2))))
        # log_p_v = logdet_v.sum() + log_p_sum_v
        log_p_v = torch.max(torch.zeros_like(log_p_v),
                            (-log_p_v / (np.log(2) * input_embed_dict['rgb'].size(0) * input_embed_dict['rgb'].size(1) * input_embed_dict['rgb'].size(2))))
          
        if self.missing_mode == 1:
            x = self.non_rgb_modalities[0]
            if comb_random >= 0.5:
                #  normalizing flow for x
                # [to do]: check unified prior
                # _, logdet_x, z_outs_x = self.flow_x(input_embed_dict[x].unsqueeze(-1))
                log_p_x, logdet_x, z_outs_x = self.flow_x(input_embed_dict[x].unsqueeze(-1))
                z_a = z_outs_x
                z_outs_x = z_outs_x[0]

                # [to do]: check unified prior
                # log_p_sum_x = gaussian_log_p(z_outs_x.unsqueeze(0), mean, log_sd).view(input_text_embeds.size(0), -1).sum(1)
                # log_p_x = logdet_x.sum() + log_p_sum_x
                log_p_x = torch.max(torch.zeros_like(log_p_x),
                                    (-log_p_x / (np.log(2) * input_embed_dict[x].size(0) * input_embed_dict[x].size(1) * input_embed_dict[x].size(2))))

                t2x = self.flow_x.reverse(z_t, reconstruct=True).squeeze(-1).detach()
                v2x = self.flow_x.reverse(z_v, reconstruct=True).squeeze(-1).detach()
                x_gen = self.rec_x(self.cat_x(torch.cat([t2x, v2x], dim=1)))
                loss_rec = self.MSE(x_gen, input_embed_dict[x].detach())
            else:
                log_p_x, loss_rec = 0.0, 0.0
                t2x = self.flow_x.reverse(z_t, reconstruct=True).squeeze(-1).detach()
                v2x = self.flow_x.reverse(z_v, reconstruct=True).squeeze(-1).detach()
                x_gen = self.rec_x(self.cat_x(torch.cat([t2x, v2x], dim=1)))
                input_embed_dict[x] = x_gen
            loss_gen = log_p_t + log_p_v + log_p_x + loss_rec
        else:
            loss_gen = 0
        ...
        return {'loss': loss+MI_loss+loss_gen}
