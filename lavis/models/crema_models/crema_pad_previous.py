...
@registry.register_model("crema_pad_previous")
class CREMA_PAD(Blip2Base):
    ...
    def forward(self, samples):
        ...
        # t5_inputs['rgb'], t5_atts['rgb'], t5_query['rgb'] = self.get_qformer_embedding(input_embed_dict['rgb'], input_att_dict['rgb'], device, 'rgb', b)
        # if comb_random >= 0.5:
        for modal in self.modalities:
        ...
        if 'rgb' in self.modalities:
            ...
            for modal in self.modalities:
                if modal == 'rgb':
                    continue
                if modal in ['depth', 'norm', 'flow'] and comb_random >= 0.5:
                    if 'espresso' in self.task:
                ...
                if 'espresso' in self.task:
                    ...
                    # if True:
                        ...
                        if 'concat' in self.task:
                            ...
                            pad_token_id = self.t5_tokenizer.pad_token_id 
                            # padding = torch.full((bs, padding_length), 
                            #                 pad_token_id, 
                            #                 dtype=torch.long,
                            #                 device=device)
                            # inputs_t5_extra_padding = self.t5_model.encoder.embed_tokens(padding)
                            # inputs_t5_extra_padding.requires_grad = True    # need grad to concat with rgb input
                            # attention_padding = torch.zeros((inputs_t5_extra_padding.size(0), padding_length), 
                            #                     dtype=torch.long,
                            #                     device=device)
                            # inputs_t5 = torch.cat([inputs_t5, inputs_t5_extra_padding], dim=1)
                            # atts_t5 = torch.cat([atts_t5, attention_padding], dim=1)
                            
                            # 直接创建一个与inputs_t5相同类型的零张量
                            padding_embed_shape = (inputs_t5.size(0), inputs_t5_rgb.size(1)*inputs_t5_rgb.size(2), inputs_t5.size(2))
                            inputs_t5_extra_padding = torch.zeros(padding_embed_shape, 
                                                                dtype=inputs_t5.dtype,
                                                                device=inputs_t5.device,
                                                                requires_grad=inputs_t5.requires_grad)
                            
                            # 创建注意力掩码
                            attention_padding = torch.zeros((inputs_t5.size(0), padding_embed_shape[1]), 
                                                dtype=atts_t5.dtype,
                                                device=atts_t5.device)
