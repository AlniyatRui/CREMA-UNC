...
@registry.register_model("crema_pad")
class CREMA_PAD(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        init_vec = self.t5_model.encoder.embed_tokens.weight[pad_id].detach().clone()
        pad_embedding_weight = nn.Parameter(init_vec.unsqueeze(0))  # (1, d_model)
        self.register_parameter(f'pad_embedding_weight', pad_embedding_weight)
    ...
    def forward(self, samples):
        ...
        # print(f'self.pad_embedding_weight.requires_grad {self.pad_embedding_weight.requires_grad}')
        ...
        if 'rgb' in self.modalities:
            ...
            if 'audio' not in self.modalities and 'pc' not in self.modalities:
                if 'espresso' in self.task:
                    if (self.missing_mode == 1 and comb_random >= 0.5)or (self.missing_mode == 2 and comb_random >= 0.75) or (self.missing_mode == 3 and comb_random >= 7/8):
                        # vtx full modality
                        ...
                        if 'concat' in self.task:
                            ...
                            raise NotImplementedError
                    elif self.missing_mode == 1:
                        # vt no x, pad fusion output
                        if 'concat' in self.task:
                            inputs_t5 = inputs_t5.reshape(b, -1, inputs_t5.shape[-1]) #torch.Size([16, 144, 2048]), bs,frame*qformer,dim
                            atts_t5 = atts_t5.reshape(b, -1)

                            bs = inputs_t5.size(0)
                            padding_length = inputs_t5_rgb.size(1)*inputs_t5_rgb.size(2)    # no vid_prefix_embed
                            
                            pad_vec = self.pad_embedding_weight      # (1, d)
                            inputs_t5_extra_padding = pad_vec.expand(bs, padding_length, -1)
                            attention_padding = torch.zeros(bs, padding_length, dtype=atts_t5.dtype, device=device)
                            
                            inputs_t5 = torch.cat([inputs_t5, inputs_t5_extra_padding.to(inputs_t5.dtype)], dim=1)
                            atts_t5 = torch.cat([atts_t5, attention_padding], dim=1)
                        ...
                        if 'concat' in self.task:
                            ...
                            missing_modal_num = 2 - len(fusion_modal)
                            if missing_modal_num:
                                padding = pad_vec.expand(bs, inputs_t5_rgb.size(1),inputs_t5_rgb.size(2), -1)
                                inputs_t5_extra_padding = [padding for i in range(missing_modal_num)]
                                fusion_modal += inputs_t5_extra_padding
                            fusion_modal = torch.cat(fusion_modal, dim=-1) 
                            inputs_t5_extra = self.fusion(fusion_modal) #same dim as fusion modal
                            inputs_t5_extra = self.sigmoid(inputs_t5_extra) * inputs_t5_extra #activate 
                            vpe = vid_prefix_embed.reshape(b, -1, vid_prefix_embed.shape[-1]) #bs,frame*qformer,dim
                        ...
                        if 'concat' in self.task:
                            ...
                            missing_modal_num = 3 - len(fusion_modal)
            ...
            # [F1, F2, F3,..., A]
                ...
                if 'espresso' in self.task:                                        
                    if self.missing_mode == 4 and comb_random >= 15/16:
                        # vtx full modality
                        fusion_modal = torch.cat(fusion_modal, dim=-1) 
                        inputs_t5_extra = self.fusion(fusion_modal) #same dim as fusion modal
                        ...
                        if 'concat' in self.task:
                            ...
                            missing_modal_num = 4 - len(fusion_modal)
                ...
                if 'espresso' in self.task:                                        
                    if self.missing_mode == 3 and comb_random >= 7/8:
                        # vtx full modality
                        fusion_modal = torch.cat(fusion_modal, dim=-1) 
                        inputs_t5_extra = self.fusion(fusion_modal) #same dim as fusion modal
    ...
    def generate(self,
        ...
        input_embed_dict, input_att_dict = {}, {}
        x, y, z = self.non_rgb_modalities
        if 'comb_random' in samples:
            comb_random = samples['comb_random']
        else:
            comb_random = 7/8 + 0.01
