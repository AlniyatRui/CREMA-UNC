...
class CREMA_GEN_v1(Blip2Base):
    ...
    def generate(self,
        ...
        if comb_random < 7/8:
            ...
            #     dist.barrier()  
            # x_bs = 16 # pc
            x_bs = input_embed_dict['rgb'].size(0) #64
            t = input_text_embeds.repeat(int(x_bs/t_bs),1,1).transpose(1, 2).to(self.query_tokens_flow.data.dtype)
            v = input_embed_dict['rgb'].transpose(1, 2).to(t.dtype)
            v_bs = v.size(0)
            # v_forx = v.view(x_bs, int(v_bs/x_bs), v.size(-2), v.size(-1)).mean(dim=1).to(t.dtype)
